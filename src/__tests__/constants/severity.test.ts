...
describe('severity filter constant', () => {
    it.each(methods)('should be the correct value', async (method) => {
        const expected = method;

        const { entries, log } = createLogWithFilter(`severity == ${JSON.stringify(expected)}`);
