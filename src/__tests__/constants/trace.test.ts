...
describe('trace filter constant', () => {
    it('should be the value of LogType.Trace', async () => {
        const expected = LogType.Trace;

        const { entries, log } = createLogWithFilter(`trace == ${JSON.stringify(expected)}`);
