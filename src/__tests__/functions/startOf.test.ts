...
describe('startOf() filter function', () => {
    it.each(units)('should get correct start of time value', async (unit: any) => {
        const time = now.clone();
        const expected = time.startOf(unit).valueOf();

        const { entries, log } = createLogWithFilter(`startOf(now()) == ${JSON.stringify(expected)}`);
