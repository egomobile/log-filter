...
describe('concat() filter function', () => {
    it.each(values)('should convert items to strings and join them to one without a separator', async ({ args }) => {
        const expected = args.join('');
        const argList = args.map(a => JSON.stringify(a)).join(',');

        const { entries, log } = createLogWithFilter(`concat(${argList}) == ${JSON.stringify(expected)}`);
