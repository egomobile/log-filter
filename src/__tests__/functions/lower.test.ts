...
describe('lower() filter function', () => {
    it.each(values)('should convert input value to lower chars', async (value) => {
        const expected = String(value).toLowerCase();

        const { entries, log } = createLogWithFilter(`lower(arg0) == ${JSON.stringify(expected)}`);
