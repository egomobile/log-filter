...
describe('isoWeekday() filter function', () => {
    it('should get correct ISO weekday', async () => {
        const time = now.clone();
        const expected = time.isoWeekday();

        const { entries, log } = createLogWithFilter(`isoWeekday() == ${JSON.stringify(expected)}`);
