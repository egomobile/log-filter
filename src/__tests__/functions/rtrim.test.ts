...
describe('rtrim() filter function', () => {
    it.each(values)('should removing ending whitespace chars', async (value) => {
        const expected = String(value).trimEnd();

        const { entries, log } = createLogWithFilter(`rtrim(arg0) == ${JSON.stringify(expected)}`);
