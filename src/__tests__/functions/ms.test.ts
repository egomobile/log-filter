...
describe('ms() filter function', () => {
    it('should get correct millisecond', async () => {
        const time = now.clone();
        const expected = time.millisecond();

        const { entries, log } = createLogWithFilter(`ms() == ${JSON.stringify(expected)}`);
