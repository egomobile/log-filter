...
describe('ltrim() filter function', () => {
    it.each(values)('should removing leading whitespace chars', async (value) => {
        const expected = String(value).trimStart();

        const { entries, log } = createLogWithFilter(`ltrim(arg0) == ${JSON.stringify(expected)}`);
